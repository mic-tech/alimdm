...
/**
 * Core MQTT client for AliMDM that integrates with Home Assistant via MQTT Discovery.
...
class KioskMqttClient(
    ...
    private val effectiveClientId: String = config.clientId?.takeIf { it.isNotBlank() } ?: "alimdm_$deviceId"
        ...
        try {
            if (wifiLock == null) {
                ...
                wifiLock = wifiManager.createWifiLock(
                    WifiManager.WIFI_MODE_FULL_HIGH_PERF, "AliMDM:MqttClient")
                wifiLock?.acquire()
            ...
            if (cpuWakeLock == null) {
                ...
                cpuWakeLock = powerManager.newWakeLock(
                    PowerManager.PARTIAL_WAKE_LOCK, "AliMDM:MqttCPU")
                cpuWakeLock?.acquire()
