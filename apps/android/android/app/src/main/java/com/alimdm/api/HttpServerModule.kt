...
import com.alimdm.DeviceAdminReceiver
import com.alimdm.KioskModule
import com.alimdm.MainActivity
import com.alimdm.CameraPhotoModule
import com.alimdm.AliMdmAccessibilityService
import com.alimdm.ScreenController
...
class HttpServerModule(private val reactContext: ReactApplicationContext) : 
    ...
    private fun acquireServerLocks() {
        try {
            ...
            wifiLock = wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "AliMDM:HttpServer")
            wifiLock?.acquire()
    ...
    private fun getDeviceStatus(): JSONObject {
        ...
        val deviceStatus = JSONObject().apply {
            ...
            put("hostname", "alimdm")
            put("version", com.alimdm.BuildConfig.VERSION_NAME)
    ...
    private fun handleCommand(
        ...
        when (command) {
            ...
            "setMode" -> {
                ...
                // AliMDM is backgrounded behind an external app the JS thread is frozen,
                // so a REST/MQTT setMode returned success while nothing happened on screen
                // until the user manually brought AliMDM forward (home/back). When switching
                // to a foreground mode (webview / media_player) we bring AliMDM to the front
                ...
                // Only intervene when AliMDM is actually backgrounded (behind an external
    ...
    /**
     * #209: Bring AliMDM's MainActivity to the foreground from native code. Used when a
    ...
    /**
     * #209: True when AliMDM itself is the current foreground app. Used to skip the native
    ...
    private fun speakText(text: String, language: String? = null, voiceName: String? = null) {
        try {
            if (tts != null && ttsReady) {
                ...
                tts?.speak(text, TextToSpeech.QUEUE_FLUSH, null, "alimdm_tts_${System.currentTimeMillis()}")
    ...
    private fun dispatchKeyWithMeta(keyCode: Int, metaState: Int) {
        // Try AccessibilityService first (works cross-app on all ROMs)
        if (AliMdmAccessibilityService.isRunning()) {
            AliMdmAccessibilityService.sendKeyWithMeta(keyCode, metaState)
            return
        }
        // Fallback to Activity dispatchKeyEvent (works only in AliMDM's own Activity)
        UiThreadUtil.runOnUiThread {
    ...
    private fun sendKeyboardText(text: String): JSONObject {
        // Try AccessibilityService first (works cross-app on all ROMs)
        if (AliMdmAccessibilityService.isRunning()) {
            AliMdmAccessibilityService.sendText(text)
            return JSONObject().apply {
                put("executed", true)
     ...
     *   Device Owner screen-capture policy, but it only works while AliMDM is on
     ...
     *   "Window doesn't have a backing surface!" (and would capture an empty AliMDM
    ...
    /**
     * PixelCopy on the AliMDM Activity window. Captures hardware-accelerated layers
            ...
            UiThreadUtil.runOnUiThread {
                try {
                    ...
                    if (window != null && decorView != null &&
                        ...
                        lastScreenshotError = "AliMDM window is not on screen"
                    ...
                    lastScreenshotError = "AliMDM window is not on screen (${e.message})"
    ...
    private fun captureViaAccessibility(): java.io.InputStream? {
        ...
        if (!AliMdmAccessibilityService.canTakeScreenshot()) {
            lastScreenshotError = "Accessibility service is enabled but has no screenshot capability: " +
                "disable and re-enable AliMDM in Android accessibility settings"
            return null
        }
        ...
        return try {
            ...
            var bitmap = AliMdmAccessibilityService.captureScreen()
            if (bitmap != null && policyLifted && isBlankFrame(bitmap)) {
                ...
                bitmap = AliMdmAccessibilityService.captureScreen()
            }
            if (bitmap == null) {
