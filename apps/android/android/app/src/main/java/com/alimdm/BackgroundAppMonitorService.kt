...
class BackgroundAppMonitorService : Service() {
    ...
    companion object {
        ...
        private const val CHANNEL_ID = "alimdm_bg_monitor"
    ...
    private fun checkAndRelaunchApps() {
        // Don't relaunch if AliMDM is NOT in the foreground
        // (user might be in Settings, PIN screen, or another flow)
        if (!isAliMdmInForeground()) {
            DebugLog.d(TAG, "AliMDM is not in foreground, skipping relaunch check")
            return
        }
     ...
     * brings AliMDM back to front after a short delay. The delay is kept
    ...
    private fun relaunchApp(packageName: String) {
        try {
            ...
            if (launchIntent != null) {
                ...
                DebugLog.d(TAG, "Relaunched: $packageName — bringing AliMDM back to front")
                // Bring AliMDM back to the foreground after a short delay
                handler.postDelayed({ bringAliMdmToFront() }, 300)
            } else {
    ...
    /**
     * Bring AliMDM's MainActivity back to the foreground without recreating it.
     */
    private fun bringAliMdmToFront() {
        try {
            ...
            DebugLog.d(TAG, "AliMDM brought back to front")
        } catch (e: Exception) {
            DebugLog.d(TAG, "Failed to bring AliMDM to front: ${e.message}")
        }
    }
