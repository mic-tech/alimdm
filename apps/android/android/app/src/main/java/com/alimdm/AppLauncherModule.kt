        ...
        fun checkAndBroadcast() {
            try {
                ...
                if (topPackage == packageName) {
                    ...
                    reactApplicationContext.sendBroadcast(Intent("com.alimdm.EXTERNAL_APP_LAUNCHED").apply {
                        putExtra("package_name", packageName)
                        putExtra("verified", true)
                    })
                    android.util.Log.i("AliMDM-ADB", "EXTERNAL_APP_LAUNCHED: $packageName (verified in foreground)")
                } else if (retryCount < maxRetries) {
                    // App not yet in foreground, retry
                    retryCount++
                    android.util.Log.d("AliMDM-ADB", "Waiting for $packageName to be in foreground (attempt $retryCount/$maxRetries, current: $topPackage)")
                    ...
                    android.util.Log.w("AliMDM-ADB", "EXTERNAL_APP_LAUNCHED: $packageName (NOT verified - timeout after ${maxRetries * retryDelayMs}ms, top: $topPackage)")
                }
            } catch (e: Exception) {
                android.util.Log.e("AliMDM-ADB", "Error checking foreground for EXTERNAL_APP_LAUNCHED: ${e.message}")
                ...
                try {
                    ...
                    android.util.Log.i("AliMDM-ADB", "EXTERNAL_APP_LAUNCHED: $packageName (fallback - error during verification)")
                } catch (ex: Exception) {
                    android.util.Log.e("AliMDM-ADB", "Failed to broadcast EXTERNAL_APP_LAUNCHED: ${ex.message}")
                }
            }
        }
    ...
    fun launchBootApps(promise: Promise) {
        executor.execute {
            try {
                ...
                // Do NOT bring AliMDM to front here. loadSettings() will call
                // launchExternalApp() right after, which properly starts OverlayService
                // and launches the primary app. Bringing AliMDM back in between would
