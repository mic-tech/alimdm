...
# Ali MDM Device Owner Setup Script
# This script helps set up Device Owner mode for Ali MDM app
...
clear
echo "${GREEN}╔═══════════════════════════════════════════════╗${NC}"
echo "${GREEN}║   Ali MDM Device Owner Setup Assistant     ║${NC}"
echo "${GREEN}╚═══════════════════════════════════════════════╝${NC}\n"
...
# Step 1: Find APK
print_step "Step 1: Locating Ali MDM APK"
APK_FILE=$(find "$SCRIPT_DIR" -maxdepth 1 -name "alimdm-v*.apk" | head -n 1)

if [[ -z "$APK_FILE" ]]; then
    print_error "No Ali MDM APK found in the same directory as this script"
    print_info "Looking for: alimdm-v*.apk"
...
# Step 4: Install APK
print_step "Step 4: Installing Ali MDM APK"
...
DPM_OUTPUT=$(adb shell dpm set-device-owner com.alimdm/.DeviceAdminReceiver 2>&1)
echo "$DPM_OUTPUT"
echo ""
...
echo "${GREEN}╚═══════════════════════════════════════════════╝${NC}"
echo ""
print_success "Ali MDM is now configured as Device Owner"
print_info "You can now configure kiosk settings in the Ali MDM app"
echo ""
