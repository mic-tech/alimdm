    ...
    if (Platform.Version < 33) {
      const writeGranted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
        {
          ...
          message: 'Ali MDM needs storage access to save and load backup files.',
...
function generateBackupFilename(): string {
  ...
  return `alimdm-backup-${timestamp}.json`;
}
...
export async function listBackupFiles(): Promise<{ name: string; path: string; date: string }[]> {
  try {
    ...
    const backupFiles = files
      .filter(file => file.name.startsWith('alimdm-backup-') && file.name.endsWith('.json'))
      .map(file => ({
        name: file.name,
        path: file.path,
