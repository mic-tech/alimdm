...
const ExternalAppOverlay: React.FC<ExternalAppOverlayProps> = ({
  ...
  // Tracks which managed apps are actually installed yet. Apps that are in the
  // config but not installed show a "Downloading..." state until the cloud
  // install_apk command completes, so a kid tapping a not-yet-installed tile
  // gets clear feedback instead of a dead tap.
  ...
  useEffect(() => {
    const resolveLabelsAndIcons = async () => {
      ...
      setAppLabels(labels);
      setAppIcons(icons);
      // Check which apps are installed yet.
      const installed: Record<string, boolean> = {};
      await Promise.all(appsToResolve.map(async (app) => {
        try {
          installed[app.packageName] = await AppLauncherModule.isAppInstalled(app.packageName);
        } catch {
          installed[app.packageName] = false;
        }
      }));
      setAppInstalled(installed);
      // Re-check install status every 5s while any app is still missing, so the
      // "Downloading..." badge flips to the real icon as soon as the cloud
      // install finishes — no manual refresh needed.
      const hasMissing = appsToResolve.some((a) => !installed[a.packageName]);
      if (hasMissing) {
        recheckTimer.current = setTimeout(resolveLabelsAndIcons, 5000);
      } else if (recheckTimer.current) {
  ...
  const renderAppIcon = ({ item }: { item: ManagedApp }) => {
    ...
    const isInstalled = appInstalled[item.packageName] !== false;
    const showDownloading = isMultiAppMode && !isInstalled;

    return (
      <TouchableOpacity
        style={[styles.appIconContainer, { width: appTileWidth }, showDownloading && { opacity: 0.6 }]}
        onPress={() => { if (!showDownloading) handleAppPress(item.packageName); }}
        activeOpacity={0.7}
        disabled={showDownloading}
      >
        {iconUri && !showDownloading ? (
          <Image
            source={{ uri: iconUri }}
            ...
            <View style={styles.warningContainer}>
              <View style={styles.warningCard}>
                ...
                <Text style={styles.warningText}>
                  You can use the Android back button to return to Ali MDM.
                </Text>
              </View>
            </View>
          )}
