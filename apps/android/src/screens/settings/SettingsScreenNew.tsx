/**
 * Ali MDM v1.2 - New Settings Screen
...
const SettingsScreenNew: React.FC<SettingsScreenProps> = ({ navigation }) => {
  ...
  // #199 — opt-in: when ON and a native screen-lock is set, Ali MDM steps aside for the
  ...
  // #199 — opt-in: make Ali MDM the Home launcher so the system relaunches it after
  ...
  const toggleDefaultLauncher = async (value: boolean) => {
    ...
    try {
      if (isDeviceOwner) {
        ...
        // No Device Owner: send the user to the system Home-app picker to choose Ali MDM.
    ...
    try {
      ...
      if (error?.code === 'INSTALL_PERMISSION' || errorMsg.includes('unknown sources')) {
        Alert.alert(
          ...
          'The update was downloaded but cannot be installed.\n\nPlease enable "Install from unknown sources" for Ali MDM in your device settings, then try again.\n\nOn restricted devices (e.g. Echo Show), use:\nadb install -r <apk>',
        );
      } else {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                'Success',
                'Device Owner removed!\n\nYou can now uninstall Ali MDM normally.',
                [{ text: 'OK', onPress: () => { revokeSettingsAccess(); navigation.reset({ index: 0, routes: [{ name: 'Kiosk' }] }); } }]
              );
            } catch (error: any) {
