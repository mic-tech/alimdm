        ...
        <SettingsSection title="Brightness Control" icon="brightness-6">
          <SettingsSwitch
            ...
            hint={brightnessManagementEnabled
              ? "Ali MDM manages screen brightness"
          ...
          {!brightnessManagementEnabled && (
            <SettingsInfoBox variant="info">
              <Text style={styles.infoText}>
                Brightness is managed by the system. External tools like Tasker can control brightness without interference from Ali MDM.
              </Text>
            </SettingsInfoBox>
          )}
        </SettingsSection>
...
`}
                  {displayMode === 'external_app'
                    ? `• Ali MDM comes to the foreground to show the screensaver; the external app resumes on wake
`
                    : ''}
      ...
      {displayMode === 'webview' && (
        <SettingsSection title="Web Media" icon="volume-off">
          ...
          <SettingsSwitch
            ...
            hint="Enable for WebRTC 2-way audio / talk-back (e.g. a Home Assistant / go2rtc doorbell intercom card). While the web page is actively using the microphone, Ali MDM switches the device to communication audio mode so the microphone back-channel transmits, then restores normal audio when you stop talking. Leave off for normal browsing — it only engages while the mic is in use."
