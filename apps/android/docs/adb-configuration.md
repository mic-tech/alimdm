...
# Ali MDM ADB Configuration Guide
...
Ali MDM supports configuration via Android Debug Bridge (ADB) intent extras, enabling:
...
> [!NOTE]
> Setting Device Owner is **highly recommended** for external app locking as it allows Ali MDM to automatically manage all required permissions including "Display over other apps". Without Device Owner, this permission must be granted manually through Settings.
...
```bash
adb shell am start -n com.alimdm/.MainActivity [OPTIONS]
```
...
| `--es back_button_mode "immediate"` | String | `"test"` | Behavior when returning to Ali MDM via Android back button: `"test"` (stay on Ali MDM, back button allowed), `"timer"` (countdown then relaunch app), `"immediate"` (instantly relaunch app). If `test_mode` is also set, `back_button_mode` takes priority |
...
| `--es mqtt_base_topic "alimdm"` | String | Base MQTT topic prefix |
...
The ADB command returns immediately, but Ali MDM needs time to save settings and restart. **Use broadcast receivers to wait for completion**:
...
```typescript
private async setupKioskModeUsingAli MDM(packageName: string): Promise<void> {
  ...
  const appLaunchMonitor = this.execAsync(
    `adb -s ${this.adbTarget} logcat -c && adb -s ${this.adbTarget} logcat -s "Ali MDM-ADB" | grep -m 1 "EXTERNAL_APP_LAUNCHED: ${packageName}"`
  );

  // Send configuration command with auto_start
  const kioskCmd = `adb -s ${this.adbTarget} shell am start -n com.alimdm/.MainActivity --es lock_package "${packageName}" --es pin "${PIN}" --ez auto_start true`;
...
adb logcat -c
adb logcat -s "Ali MDM-ADB" | grep -m 1 "SETTINGS_LOADED" &
LOGCAT_PID=$!
...
Ali MDM emits these broadcasts during ADB configuration:

| Broadcast Action | When | Description |
|-----------------|------|-------------|
| `com.alimdm.ADB_CONFIG_SAVED` | Immediately after config saved | Database write complete, restart imminent |
| `com.alimdm.ADB_CONFIG_RESTARTING` | Just before restart | Process kill in 500ms |
| `com.alimdm.SETTINGS_LOADED` | After restart complete | App restarted and settings loaded successfully |
| `com.alimdm.EXTERNAL_APP_LAUNCHED` | After external app launch (auto_start=true) | External app is launched and visible (includes package_name extra) |
...
adb logcat -c
adb logcat -s "Ali MDM-ADB" | grep -m 1 "EXTERNAL_APP_LAUNCHED: $PACKAGE" &
LOGCAT_PID=$!
...
> [!NOTE]
> `test_mode "false"` sets `back_button_mode` to `immediate`, meaning the Android back button will instantly relaunch the app. Omit it or set to `"true"` during testing to allow the back button to return to Ali MDM without auto-relaunch.
...
```bash
#!/bin/bash
# provision_kiosk.sh - Provision a Ali MDM device
...
```powershell
# provision_kiosk.ps1 - Provision a Ali MDM device (Windows)
...
Write-Host "Configuring Ali MDM..."
...
**Cause**: Ali MDM requires "Display over other apps" (SYSTEM_ALERT_WINDOW) permission to keep external apps locked.
...
adb shell dpm set-device-owner com.alimdm/.DeviceAdminReceiver
```
With Device Owner, Ali MDM can automatically grant itself all required permissions including "appear on top" and "Usage Access".

**Option 2: Manual permission (without Device Owner)**
1. Go to Settings → Apps → Ali MDM → Display over other apps
2. Enable the permission manually

### EXTERNAL_APP_LAUNCHED not appearing in logcat

**Cause**: Without Device Owner, the "Usage Access" (PACKAGE_USAGE_STATS) permission must be granted manually for Ali MDM to verify that the external app is in the foreground.

**Solution**: Grant the permission via ADB:
```bash
adb shell appops set com.alimdm android:get_usage_stats allow
```
Without this permission, the broadcast will still be emitted but with `(NOT verified)` instead of `(verified in foreground)`. The logcat tag is `Ali MDM-ADB`:
```bash
adb logcat -s "Ali MDM-ADB"
```
...
Alternatively, you can enable the Accessibility Service manually: Settings → Accessibility → Installed Services → Ali MDM.
...
# Option 2: Remove Device Owner + clear Ali MDM data
adb shell dpm remove-active-admin com.alimdm/.DeviceAdminReceiver
adb shell pm clear com.alimdm
```
