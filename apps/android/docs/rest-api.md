...
# Ali MDM REST API Documentation
...
Ali MDM includes a built-in REST API server for integration with **Home Assistant** and other smart home platforms.
...
> foreground, Ali MDM's own window is off screen and cannot be copied, so the capture goes through the
> **accessibility service** instead (`Settings > Accessibility > Ali MDM`, Android 11+ required). In addition,
> Lock Mode as Device Owner blocks screen capture device-wide (this is what disables the Power+Volume Down
> combo), which also blacks out that capture: enable **Security > Allow Remote Screenshots** to let Ali MDM
...
> 
> **Ali MDM uses a 4-tier approach for screen off:**  
...
>
> This means Ali MDM can coexist with an existing MDM that holds Device Owner — just activate Device Admin or the AccessibilityService and screen lock works fully.
> 
...
Restart the Ali MDM app UI. Calls `activity.recreate()` to fully restart the React Native activity without rebooting the device. Useful for troubleshooting UI issues remotely.
...
> **How it works**: Remote key events are dispatched natively using the **AccessibilityService** (cross-app, works in external apps) or `activity.dispatchKeyEvent()` (fallback, Ali MDM only). When the AccessibilityService is enabled:
...
>
> Without the AccessibilityService, keys only work within Ali MDM's own Activity.
...
>
> By default, keyboard emulation only works inside Ali MDM's WebView. To inject keys into **external apps** (e.g., when using External App display mode), you need to enable the **Ali MDM Accessibility Service**:
>
> 1. Go to **Settings → Advanced → Accessibility Service** and tap **"Open Accessibility Settings"**
> 2. In Android settings, find **Ali MDM** under "Installed Services" and enable it
> 3. The status indicator in Ali MDM settings will show **"● Active"** when ready
>
> **Device Owner shortcut**: If Ali MDM is set as Device Owner, you can enable the service automatically without visiting Android settings — just tap **"Enable Automatically"** in Advanced Settings.
>
> **How it works**: When the Accessibility Service is active, Ali MDM uses it for all keyboard injection (both WebView and external apps). When disabled, it falls back to `dispatchKeyEvent()` which only works within Ali MDM's own Activity.
>
